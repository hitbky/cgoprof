...
func fillDBError(dbErr *Error, db *C.sqlite3) {
	__cgoprof_end_1 :=
		...
		prof.BeginCall("13f44204c8", "sqlite3_errcode")
	__cgoprof_ret_2 := C.sqlite3_errcode(db)
	__cgoprof_end_1()
	dbErr.Code = ErrNo(__cgoprof_ret_2)
	__cgoprof_end_3 := prof.BeginCall("460b942ffa", "sqlite3_extended_errcode")
	__cgoprof_ret_4 := C.sqlite3_extended_errcode(db)
	__cgoprof_end_3()
	dbErr.ExtendedCode = ErrNoExtended(__cgoprof_ret_4)
	__cgoprof_end_5 := prof.BeginCall("9b9c25a1b3", "sqlite3_errmsg")
	__cgoprof_ret_6 := C.sqlite3_errmsg(db)
	__cgoprof_end_5()
	prof.Conversion("f0f73028d4", "C.GoString", int(0))
	__cgoprof_end_7 := prof.BeginCall("f0f73028d4", "GoString")
	__cgoprof_ret_8 := C.GoString(__cgoprof_ret_6)
	__cgoprof_end_7()
	dbErr.err = __cgoprof_ret_8
}
...
func fillExpandedSQL(info *TraceInfo, db *C.sqlite3, pStmt unsafe.Pointer) {
	...
	__cgoprof_end_9 := prof.BeginCall("d953471f4b", "sqlite3_expanded_sql")
	__cgoprof_ret_10 := C.sqlite3_expanded_sql((*C.sqlite3_stmt)(pStmt))
	__cgoprof_end_9()
	expSQLiteCStr := __cgoprof_ret_10
	defer func() {
		__cgoprof_end_11 := prof.BeginCall("eb92baa133", "sqlite3_free")
		C.sqlite3_free(unsafe.Pointer(expSQLiteCStr))
		__cgoprof_end_11()
	}()
	...
	prof.Conversion("1b5f0c2b16", "C.GoString", int(0))
	__cgoprof_end_12 :=

		prof.BeginCall("1b5f0c2b16", "GoString")
	__cgoprof_ret_13 := C.GoString(expSQLiteCStr)
	__cgoprof_end_12()
	info.ExpandedSQL = __cgoprof_ret_13
}
...
func traceCallbackTrampoline(
	...
	defer func() {
		prof.Callback("b9446ec49b", "C", time.Since(__cgoprof_callbackStart_14))
	}()
	...
	__cgoprof_end_15 := prof.BeginCall("3b42bdc9df", "sqlite3_get_autocommit")
	__cgoprof_ret_16 := C.sqlite3_get_autocommit(contextDB)
	__cgoprof_end_15()
	info.AutoCommit = (int(__cgoprof_ret_16) != 0)
	...
	case TraceStmt:
		...
		var xStr string
		if xValue != nil {
			prof.Conversion("ae8402c7c3", "C.GoString", int(0))
			__cgoprof_end_17 := prof.BeginCall("ae8402c7c3", "GoString")
			__cgoprof_ret_18 := C.GoString((*C.char)(xValue))
			__cgoprof_end_17()
			xStr = __cgoprof_ret_18
		}
...
func (c *SQLiteConn) setSQLiteTrace(sqliteEventMask uint) error {
	__cgoprof_end_19 := prof.BeginCall("639802ecfe", "sqlite3_trace_v2")
	__cgoprof_ret_20 := C.sqlite3_trace_v2(c.db,
		C.uint(sqliteEventMask),
		(*[0]byte)(unsafe.Pointer(C.traceCallbackTrampoline)),
		unsafe.Pointer(c.db))
	__cgoprof_end_19( // Fourth arg is same as first: we are
	)
	rv := __cgoprof_ret_20
