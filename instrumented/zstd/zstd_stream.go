...
func NewWriterLevelDict(w io.Writer, level int, dict []byte) *Writer {
	var err error
	__cgoprof_end_1 := prof.BeginCall("c25198149a",

		// Load dictionnary if any
		"ZSTD_createCStream")
	__cgoprof_ret_2 := C.ZSTD_createCStream()
	__cgoprof_end_1()
	ctx := __cgoprof_ret_2

	if dict != nil {
		prof.PointerCheck("4f703eb30e", 50)
		__cgoprof_end_3 := prof.BeginCall("4f703eb30e", "ZSTD_CCtx_loadDictionary")
		__cgoprof_ret_4 := C.ZSTD_CCtx_loadDictionary(ctx,
			unsafe.Pointer(&dict[0]),
			C.size_t(len(dict)),
		)
		__cgoprof_end_3()
		err = getError(int(__cgoprof_ret_4))
	}

	if err == nil {
		__cgoprof_end_5 :=
			// Only set level if the ctx is not in error already
			prof.BeginCall("2d74b22f1d", "ZSTD_CCtx_setParameter")
		__cgoprof_ret_6 := C.ZSTD_CCtx_setParameter(ctx, C.ZSTD_c_compressionLevel, C.int(level))
		__cgoprof_end_5()
		err = getError(int(__cgoprof_ret_6))
	}
...
func finalizeWriter(w *Writer) {
	if w.ctx != nil {
		__cgoprof_end_7 := prof.BeginCall("6032c5f2d0",
...
func (w *Writer) Write(p []byte) (int, error) {
	...
	consumed := 0
	for len(p) > 0 {
		prof.PointerCheck("28ac715d93", 50)
		__cgoprof_end_8 := prof.BeginCall("28ac715d93", "ZSTD_compressStream2_wrapper")
...
func (w *Writer) Flush() error {
	...
	for ret > 0 {
		prof.PointerCheck("a70c41d2ab", 50)
		__cgoprof_end_9 := prof.BeginCall("a70c41d2ab", "ZSTD_compressStream2_flush")
...
func (w *Writer) Close() error {
	...
	if w.firstError != nil {
		__cgoprof_end_10 := prof.BeginCall("59a24747b6", "ZSTD_freeCStream")
		C.ZSTD_freeCStream(w.ctx)
		__cgoprof_end_10()
		w.ctx = nil
		return w.firstError
	...
	ret := 1 // So we loop at least once
	for ret > 0 {
		prof.PointerCheck("c2db3a41ca", 50)
		__cgoprof_end_11 := prof.BeginCall("c2db3a41ca", "ZSTD_compressStream2_finish")
		...
		ret = int(w.resultBuffer.return_code)
		if err := getError(ret); err != nil {
			__cgoprof_end_12 := prof.BeginCall("ee1dacb979", "ZSTD_freeCStream")
			C.ZSTD_freeCStream(w.ctx)
			__cgoprof_end_12()
			w.ctx = nil
			return err
		...
		_, err := w.underlyingWriter.Write(w.dstBuffer[:written])
		if err != nil {
			__cgoprof_end_13 := prof.BeginCall("5c404ee783", "ZSTD_freeCStream")
			C.ZSTD_freeCStream(w.ctx)
			__cgoprof_end_13()
			w.ctx = nil
			return err
	...
	__cgoprof_end_14 := prof.BeginCall("193a303c34", "ZSTD_freeCStream")
	__cgoprof_ret_15 := C.ZSTD_freeCStream(w.ctx)
	__cgoprof_end_14()
	err := getError(int(__cgoprof_ret_15))
	w.ctx = nil
	return err
}
...
func NewReaderDict(r io.Reader, dict []byte) io.ReadCloser {
	var err error
	__cgoprof_end_16 := prof.BeginCall("a108991e46", "ZSTD_createDStream")
	__cgoprof_ret_17 := C.ZSTD_createDStream()
	__cgoprof_end_16()
	ctx := __cgoprof_ret_17
	if len(dict) == 0 {
		__cgoprof_end_18 := prof.BeginCall("cf69623622", "ZSTD_initDStream")
		__cgoprof_ret_19 := C.ZSTD_initDStream(ctx)
		__cgoprof_end_18()
		err = getError(int(__cgoprof_ret_19))
	} else {
		__cgoprof_end_20 := prof.BeginCall("03a0840d77", "ZSTD_DCtx_reset")
		__cgoprof_ret_21 := C.ZSTD_DCtx_reset(ctx, C.ZSTD_reset_session_only)
		__cgoprof_end_20()
		err = getError(int(__cgoprof_ret_21))
		if err == nil {
			prof.
				// Only load dictionary if we succesfully inited the context
				PointerCheck("341ab35b5a", 50)
			__cgoprof_end_22 := prof.BeginCall("341ab35b5a", "ZSTD_DCtx_loadDictionary")
			__cgoprof_ret_23 := C.ZSTD_DCtx_loadDictionary(
				ctx,
				unsafe.Pointer(&dict[0]),
				C.size_t(len(dict)))
			__cgoprof_end_22()
			err = getError(int(__cgoprof_ret_23))
		}
	}
...
func finalizeReader(r *reader) {
	if r.ctx != nil {
		__cgoprof_end_24 := prof.BeginCall("8b90fa185d",
...
func (r *reader) Close() error {
	...
	if r.firstError != nil {
		__cgoprof_end_25 := prof.BeginCall("19e7b8563f", "ZSTD_freeDStream")
		C.ZSTD_freeDStream(r.ctx)
		__cgoprof_end_25()
		r.ctx = nil
		return r.firstError
	...
	cPool.Put(&cb)
	dPool.Put(&db)
	__cgoprof_end_26 := prof.BeginCall("95a5b67edd", "ZSTD_freeDStream")
	__cgoprof_ret_27 := C.ZSTD_freeDStream(r.ctx)
	__cgoprof_end_26()
	err := getError(int(__cgoprof_ret_27))
	r.ctx = nil
	return err
}
...
func (r *reader) Read(p []byte) (int, error) {
	...
	for {
		...
		prof.PointerCheck("8b78ed2292", 50)
		__cgoprof_end_28 := prof.BeginCall("8b78ed2292", "ZSTD_decompressStream_wrapper")
