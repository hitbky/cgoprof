...
func (c *ctx) CompressLevel(dst, src []byte, level int) ([]byte, error) {
	...
	if len(src) == 0 {
		prof.PointerCheck("0eebe47a29", 50)
		__cgoprof_end_1 := prof.BeginCall("0eebe47a29", "ZSTD_compressCCtx")
		__cgoprof_ret_2 := C.ZSTD_compressCCtx(
			c.cctx,
			...
			C.int(level))
		__cgoprof_end_1()
		cWritten = __cgoprof_ret_2

	} else {
		prof.PointerCheck("9ae608449d", 50)
		__cgoprof_end_3 := prof.BeginCall("9ae608449d", "ZSTD_compressCCtx")
		__cgoprof_ret_4 := C.ZSTD_compressCCtx(
			c.cctx,
			unsafe.Pointer(&dst[0]),
			C.size_t(len(dst)),
...
func (c *ctx) DecompressInto(dst, src []byte) (int, error) {
	prof.PointerCheck("4eda8a8dc7", 50)
	__cgoprof_end_5 := prof.BeginCall("4eda8a8dc7", "ZSTD_decompressDCtx")
	__cgoprof_ret_6 := C.ZSTD_decompressDCtx(
		c.dctx,
		unsafe.Pointer(&dst[0]),
		C.size_t(len(dst)),
		unsafe.Pointer(&src[0]),
		C.size_t(len(src)))
	__cgoprof_end_5()
	written := int(__cgoprof_ret_6)
	err := getError(written)
	return written, err
}

func finalizeCtx(c *ctx) {
	__cgoprof_end_7 := prof.BeginCall("d64ed87637", "ZSTD_freeCCtx")
	C.ZSTD_freeCCtx(c.cctx)
	__cgoprof_end_7()
	__cgoprof_end_8 := prof.BeginCall("66ef532682", "ZSTD_freeDCtx")
